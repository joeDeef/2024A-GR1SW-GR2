...
glm::vec3 posicionDemonio = posicionModelos[1];
glm::vec3 posicionNave = glm::vec3(-0.5f, 1.5f, 0.0f);
glm::vec3 poscionLuna = posicionModelos[1] + glm::vec3(50.0f, 40.0f, 0.0f);
glm::vec3 ballPos = posicionDemonio;
glm::mat4 projection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 0.1f, 100.0f);

// Configuracion de la camara para transiciones
...
//Configuraciones para las diferentes posiciones y perspectivas de la camara
...
glm::vec3 posicionCalaveras[] = {
    ...
    glm::vec3(-4.0f, -0.1f, -2.3f),
    glm::vec3(-3.0f, -0.1f, 2.0),
    glm::vec3(2.2f, -0.1f, 3.4f),
    glm::vec3(-5.2f, -0.1f, 3.0f),
    glm::vec3(0.5f, -0.1f, 1.0f),
    glm::vec3(-0.5f, -0.1f, 2.0),
    glm::vec3(-3.0f, -0.1f, 3.0f),
    glm::vec3(5, -0.1f, -5.0),
    glm::vec3(7.0f, -0.1f, -3.0f)
};
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "The Guardian of Xibalba", NULL, NULL);
    if (window == NULL)
    {
    ...
    Model models[] = {
        ...
        Model("C:/Users/User/Documents/VisualStudio2022/OpenGL/OpenGL/model/ball/ball.obj"),
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (cambioCamara) {
            ...
            //Transición de la perspectiva y posicion de la camara
        ...
        //FIRE STONE
        if (ballPos.x < -5.0f) {
            ballPos.z = posicionDemonio.z; // Reset position
            ballPos.x = posicionDemonio.x;
            ballPos.y = posicionDemonio.y + 2;
        }
        else {
            ballPos.x -= deltaTime * 1.2f;
        }
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(ballPos)); // Move the fireball
        ...
        if (transicion == 1 || transicion == 2) {
            ...
            posicionDemonio.x = (cos(glfwGetTime() / 3.0f)) * 3.0f;
            posicionDemonio.z = (sin(glfwGetTime() / 2.0f)) * 2.0f;
        }else {
            posicionDemonio.x = (cos(glfwGetTime() / 3.0f)) * 3.0f;
            posicionDemonio.y = (sin(glfwGetTime() / 2.0f)) * 2.0f;
        }
        model = glm::mat4(1.0f);
        model = glm::translate(model, posicionDemonio);
        ...
        //SKULL
        float angulo = 45;
        for (int i = 0; i < sizeof(posicionCalaveras) / sizeof(posicionCalaveras[0]); i++) {
            model = glm::mat4(1.0f);
            if (i % 2 == 0) {
